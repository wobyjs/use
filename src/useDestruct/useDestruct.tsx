...
export const useDestruct = <TInput extends {} | []>(o: ObservableMaybe<TInput> | TInput, ...keys: (keyof TInput)[])
    ...
    const result = ks.reduce((acc, key) => {
        // Create a memoized function that properly tracks dependencies on the source observable
        acc[key as any] = useMemo(() => {
            // Access the source observable directly in the useMemo to establish dependency tracking
            const currentO = $$(o)
            const currentValue = currentO?.[key as keyof typeof currentO]

            // Handle nested observables
            return isObservable(currentValue) ? $$(currentValue) : currentValue
